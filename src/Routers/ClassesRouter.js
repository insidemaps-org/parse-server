...
export class ClassesRouter extends PromiseRouter {
  ...
  handleGet(req) {
    ...
    return rest.get(req.config, req.auth, this.className(req), req.params.objectId, options, req.info.clientSDK, req)
      .then((response) => {
  ...
  handleCreate(req) {
    return rest.create(req.config, req.auth, this.className(req), req.body, req.info.clientSDK, req);
  }

  handleUpdate(req) {
    const where = { objectId: req.params.objectId }
    return rest.update(req.config, req.auth, this.className(req), where, req.body, req.info.clientSDK, req);
  }

  handleDelete(req) {
    return rest.del(req.config, req.auth, this.className(req), req.params.objectId, req.info.clientSDK, req)
      .then(() => {
        return {response: {}};
      });
  }
