...
function handleBatch(router, req) {
  ...
  for (var restRequest of req.body.requests) {
    ...
    if (restRequest.path.startsWith("/1") === false && restRequest.path.startsWith("/parse/1") === false && restRequest.path.startsWith("/parse2/1") === false) {
    ...
    else if (restRequest.path.startsWith("/parse2/1"))
      routablePath = restRequest.path.replace(/\/parse2\/1/ig, "");
